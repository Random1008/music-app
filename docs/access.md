...
| APK — lien direct | `.../apk/hermes-music-0.3.1-debug.apk` | idem |
| Jellyfin (tailnet) | `http://100.64.0.0:8096` | Appareils connectés à Tailscale |
| Page d'installation (tailnet) | `http://100.64.0.0:8091/apk/` | idem |
| Maquette UI (tailnet) | `http://100.64.0.0:8090/` | idem |
| Réseau local | `http://192.168.0.0:<port>` | Même réseau que le NAS |

## Configuration Tailscale

```
IP tailnet : 100.64.0.0      Nom : dh4300plus-59ae.tailb74e62.ts.net
```
...
* Pour un accès « sur invitation » plutôt que public, mettre **Cloudflare Access**
  devant (le domaine est déjà chez Cloudflare) — prévu en V0.7.
